...
- **Archivos:** `panel.html` + `js/pages/panel.js` + `js/ui/class-form-modal.js` (crear/reintentar con barra de progreso, PUT directo a R2); todo reutiliza funciones ya escritas en `js/lib/api.js` (`adminListClasses/CreateUpload/SyncVideo/UpdateClass/DeleteClass`, `uploadVideoToR2`, `resizeImage`, `uploadThumbnail`) — no hizo falta backend nuevo. Auditoría de publicar/despublicar por trigger de base (`classes_audit_publish_change`, mismo patrón que `profiles_audit_role_change`) en vez de una Edge Function, porque RLS + privilegio por columna sobre `is_published` ya alcanzaban.
