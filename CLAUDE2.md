...
# npm run test:web           # (pendiente) pruebas unitarias del frontend
...
Estado a 20/09/2026. Leyenda: [x] hecho y probado · [~] hecho, con pruebas fallando o incompletas · [ ] pendiente.

- [~] Reproducibilidad de npm/Deno: versiones exactas, `deno.lock` congelado, `.nvmrc` (**falta el CI**)
- [~] Autenticación en el frontend: registro, login, logout, sesión persistente, perfil, contraseña (E2E en verde);
  **falta probar** el cambio de contraseña desde el enlace del correo
- [x] Catálogo real desde Supabase (home y videoteca; carga, error, vacío y filtros probados en navegador)
- [x] Página de detalle de clase (sin sesión, sin acceso, no encontrada, error y reintento probados)
- [~] Reproductor de un solo vídeo: HLS real con URL firmada en la ruta verificado en Chromium; **8 de 22 pruebas
  E2E fallan** (ver README, "Estado")
- [~] Pruebas: carga, error y acceso denegado ✅; URL expirada (2 de 3) y progreso (0 de 4) ⬜ pendientes de depurar
- [~] README: actualizado con el estado; faltan los pasos de despliegue definitivos
